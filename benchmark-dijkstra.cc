...
//
//
//Program to benchmark the Betweenness Centrality of a graph with weighted
//edges. Modified Dijkstra's algorithm is used to find all shortest paths.
//
//
using namespace std;
...
int main (int argc, char **argv)
{
  ...
  fout<<"Benchmark for Betweenness Centrality with weighted edges"<<endl;
  for(int i=0;i<7;i++)
    {
	...
	{
	  double serialTime = -1;
	  serialBC(graphFile,"d",serialTime);
      ...
      for(int j = 4;j<29;j +=4)
	{
	  ...
	  for(int r=i;r<7;r++)
	    {
	      double runtime = -1;
	      parallelBC(graphFile,"d",j,runtime);
	      runtimeSum+=runtime;
	    }
